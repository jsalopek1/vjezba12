<?php 
	...
	if (!isset($action)) {
		...
		if ($action == 1) {
			# dodana inicijalizacija 
			$news_title = 'US growth faster than expected.';
            setcookie('news_title_1', $news_title, time() + 3600, '/'); 
			...
			#  dodana inicijalizacija kolačića
			$news_title = 'Wall Street:Tech firm surge pushes US markets higher.';
            setcookie('news_title_2', $news_title, time() + 3600, '/');
			...
			#dodana inicijalizacija kolačića 
			$news_title = 'Hotel booking sites probed by consumer watchdog.';
            setcookie('news_title_3', $news_title, time() + 3600, '/');
